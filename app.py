...
def check_ml_anomaly(row, model, features):
    ...
    return is_ml_anomaly, ml_score, confidence, rul_days, health_index, predicted_failure_mode
...
with st.sidebar:
    ...
    **Objective:** Predict mechanical or electrical failure 
    ...
    if st.button("Clear All App Cache & Rerun", help="Clears the cache for data and resources, forcing reload of models."):
        st.cache_data.clear()
...
def get_financial_risk_level(rul_days, anomaly_count, cost_at_risk):
    ...
    risk_level = "LOW"
    risk_color = "green"
    risk_summary = "Operation is healthy. The PdM system is maintaining optimal reliability."
...
def update_plant_manager_view(kpi_ph, alert_ph, current_df, anomaly_count):
    ...
    risk_level, risk_color, risk_summary = get_financial_risk_level(rul_days, anomaly_count, cost_at_risk)
    ...
    with kpi_ph.container():
        ...
        col_kpi4.metric("Asset Health Rank", asset_rank, delta=f"HI: {st.session_state.health_index}%")
        # -----------------------------------------
        ...
        col_kpi5.metric("Financial Risk Level", f"{st_color} {risk_level}", delta=f"RUL: {rul_days} Days")
        # -------------------------------------


    with alert_ph.container():
        st.subheader("Risk-Weighted Alert, Schedule & Justification") # Updated Subheader
        ...
        if risk_color in ['red', 'orange']:
            st.markdown(f"The current Remaining Useful Life (RUL) of **{rul_days} days** dictates the following maintenance window:")
            # Highlighting the schedule for urgent planning
            st.markdown(f"<p style='background-color:#ffebeb; padding: 10px; border-radius: 5px; font-weight: bold;'>{maintenance_schedule}</p>", unsafe_allow_html=True)
        else:
        ...
        st.info(f"**PdM Value Justification:** The system delivered significant savings by preemptively identifying maintenance needs.\n\n"
    ...
    with chart_placeholder.container():
        ...
        st.plotly_chart(fig_main, use_container_width=True, key=f"pm_main_chart_{st.session_state.current_row_index}")
        # -------------------------------
...
def update_technician_view(kpi_ph, alert_ph, chart_ph, current_df, anomaly_count):
    ...
    with kpi_ph.container():
        ...
        health_delta = f"{health_delta_value:.1f}" if not pd.isna(health_delta_value) else None
        col_kpi3.metric("Health Index", f"{st.session_state.health_index}%", delta=health_delta)
        # -----------------------------
    ...
    with alert_ph.container():
        ...
        if st.session_state.rul_days <= 30 and anomaly_count > 0:
            ...
            if not anomalies.empty:
                ...
                st.markdown(f"**Predicted Failure Mode:** <span style='color: #FF4B4B; font-weight: bold;'>{predicted_mode}</span>", unsafe_allow_html=True)
                
                st.markdown(f"**RECOMMENDED ACTION:** <span style='color: #FF4B4B; font-weight: bold;'>{recommended_action}</span>", unsafe_allow_html=True)
                # -------------------------------------------
                ...
                st.markdown(f"**AI Confidence:** `{last_anomaly['AI_Confidence']:.1f}%`")
    ...
    with chart_ph.container(): # Use the allocated placeholder for the main chart
        ...
        df_failure_counts = current_df[current_df['Predicted_Failure_Mode'] != FAILURE_MODES_MAPPING['E']]['Predicted_Failure_Mode'].value_counts().reset_index()
        ...
        if not df_failure_counts.empty:
            ...
            fig_modes.update_layout(height=400, xaxis={'categoryorder':'total descending'})
            st.plotly_chart(
                fig_modes,
                use_container_width=True,
...
while st.session_state.current_row_index < len(full_data_df):
    try:
        ...
        is_ml_anomaly, ml_score, ai_confidence, rul_days_current, health_index_current, predicted_failure_mode = check_ml_anomaly( 
        ...
        if is_rule_anomaly or is_ml_anomaly:
            ...
            st.session_state.health_index = min(st.session_state.health_index, health_index_current)
            ...
            st.session_state.health_index = min(100, st.session_state.health_index + 3) # Slowly improve HI
        # ---------------------------
